...
from backend.core.config import SUPABASE_JWT_SECRET, SUPABASE_URL, SUPABASE_JWKS_URL
...
def _get_jwks_client() -> jwt.PyJWKClient | None:
    ...
    if not SUPABASE_URL:
        return None
    # Log configured SUPABASE_URL and the JWKS URL we will use to fetch keys
    try:
        logger.info(f"Configured SUPABASE_URL={SUPABASE_URL!r}")
        jwks_url = SUPABASE_JWKS_URL or f"{SUPABASE_URL.rstrip('/')}/auth/v1/.well-known/jwks.json"
