...
def validate_file(file_data:bytes, filename:str)->Tuple[bool, str, Optional[str]]:
    ...
    # Use python's built-in mimetypes to infer type based on extension (perfectly stable on Windows)
    log_info(f"Detecting file type for {filename}", context='validate_file')
    mime_type, _ = mimetypes.guess_type(filename)
    
    if not mime_type:
        # Fallback to manual extension check if mimetypes fails
    ...
    log_info(f"Detected mime type: {mime_type}", context='validate_file')
...
def _extract_pdf_with_pdfplumber(file_data: bytes) -> str:
    text = ''
    log_info("Opening PDF with pdfplumber", context='extract_pdf')
    stream = io.BytesIO(file_data)
    try:
        with pdfplumber.open(stream) as pdf:
            for i, page in enumerate(pdf.pages):
                log_info(f"Extracting page {i+1} with pdfplumber", context='extract_pdf')
...
def _extract_pdf_with_pypdf2(file_data: bytes) -> str:
    text = ''
    log_info("Opening PDF with PyPDF2", context='extract_pdf')
    stream = io.BytesIO(file_data)
    try:
        pdf_reader = PyPDF2.PdfReader(stream)
        for i, page in enumerate(pdf_reader.pages):
            log_info(f"Extracting page {i+1} with PyPDF2", context='extract_pdf')
            page_text = page.extract_text()
            if page_text:
                text += page_text + '\n'
    finally:
...
def extract_text_from_pdf(file_data: bytes) -> str:
    log_info("Selecting parser for PDF", context='extract_pdf')
    try:
        log_info("Attempting extraction with pdfplumber", context='extract_pdf')
        return _extract_pdf_with_pdfplumber(file_data)
    except Exception as e:
        log_warning(f"pdfplumber failed: {e}. Automatically retrying with PyPDF2...", context='extract_pdf')
        try:
            result = _extract_pdf_with_pypdf2(file_data)
            log_info('PDF EXTRACTION succeeded using the PyPDF2 fallback', context='resume_parser')
            return result
        except Exception as e2:
            log_error(e2, context='extract_text_from_pdf')
